...
export default function AllArticlesPage() {
  ...
  const fetchArticles = useCallback(async () => {
    ...
    try {
      ...
      const res = await fetch(`/api/projects/${id}/all?${params}`);
  ...
  function getDownloadUrl(article: Article): string {
    return article.publicUrl || article.previewUrl || article.cmsUrl || '';
  }

  async function handleDownloadHtml(article: Article) {
  ...
  async function handleDownloadAll() {
    ...
    for (const article of urlArticles) {
      setDownloading(article.id);
      try {
        await fetch(`/api/projects/${id}/html`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
    ...
    <AuthLayout>
      <div className="space-y-6">
        ...
        <div className="flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-bold">全記事一覧</h1>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-gray-500">
            {search ? `「${search}」に一致する記事がありません` : '記事がありません'}
          </div>
        ) : (
          <>
                    ...
                    <tr key={article.id} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-3 text-center">
                        <span className={`inline-block px-2 py-0.5 rounded text-xs font-medium ${
                          article.publishStatus === '公開' ? 'bg-green-100 text-green-700' :
                          article.publishFlag ? 'bg-red-100 text-red-700' :
